    ...
    public class EffectActivationEventDefine : EventDefine, IEffectEventDefine
    {
        public EffectActivationEventDefine() 
        {
            beforeVariableInfos = new EventVariableInfo[]
            {
                ...
                new EventVariableInfo() { name = VAR_EFFECT, type = typeof(IEffect)},
            };
        ...
        public override async Task execute(IEventArg arg)
        {
            ...
            if (enable)
            {
                await effect.onEnable(effectEnv);
                // 设置该Effect已被启用。
            ...
            else
            {
                ...
                await effect.onDisable(effectEnv);
            }
        }
        ...
        public static Task<EventArg> doEvent(CardEngine game, Card card, Buff buff, IEffect effect, bool enable)
        {
        ...
        IEffect IEffectEventDefine.getEffect(EventArg arg)
        {
            return arg.getVar<IEffect>(VAR_EFFECT);
        }
